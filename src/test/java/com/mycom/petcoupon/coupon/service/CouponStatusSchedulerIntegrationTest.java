...
/**
 * transitionCouponStatuses()를 직접 호출하는 CouponStatusBatchServiceImplTest와 달리,
 * 여기서는 아무것도 수동으로 부르지 않는다 — 등록된 @Scheduled cron이 전용
 * TaskScheduler(CouponBatchSchedulerConfig)를 통해 실제로 자동 실행되는지를 검증한다.
 * cron을 초 단위(매초)로 낮추고, 실행되는 즉시 통과하도록 고정 sleep 대신 Awaitility로 폴링한다.
 *
 * 실행 전 MySQL이 떠 있어야 한다: docker compose up -d mysql
 */
@SpringBootTest(properties = {
		"coupon.status.cron=*/1 * * * * *",
...
class CouponStatusSchedulerIntegrationTest {
	...
	private void setUpData() {
		...
		// 이미 발급 시작 시각이 지난 READY 쿠폰 -> 등록된 cron이 실제로 돌면 자동으로 ACTIVE가 돼야 함
