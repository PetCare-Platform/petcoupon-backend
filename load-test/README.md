...
> ⚠️ **앱과 스크립트는 같은 시간대에서 실행합니다.** 오픈·종료 시각은 스크립트를 돌리는 쪽의 로컬 시간으로 계산되고, API는 오프셋 없는 `LocalDateTime`을 받습니다. 그래서 앱 JVM의 시간대가 다르면 그 차이만큼 통째로 밀립니다 — 로컬(KST)에서 UTC로 뜬 EC2 앱을 `BASE_URL`로 때리면 "3분 뒤"가 **9시간 3분 뒤**로 저장됩니다.
>
> 스크립트는 실행 직후 오픈 시각과 함께 자기 시간대를 찍어 줍니다. 앱 로그의 시각과 다르면 그게 신호입니다.
>
> ```
> 발급 시작  2026-08-28T16:02:24 UTC+0900  (3분 뒤)
> ```
>
